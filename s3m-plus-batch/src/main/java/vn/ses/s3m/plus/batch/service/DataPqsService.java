        ...
        for (String schema : schemas) {
            ...
            try {
                ...
                if (lastData == null) {
                    ...
                    lastData.setViewTime(newData.getId().toString());
                    lastData.setSentDate(newData.getSentDate().substring(0, 4));
...
//                    }
                    ...
                    if (epCache == null) {
                        epCache = new DataPqs();
                        epCache.setDeviceId(data.getDeviceId());
                        epCache.setTCache(Double.parseDouble(data.getT() != null ? data.getT() + "" : "0.0"));
                    }
...
//                    map.put(multikey, minuteData05);
                    ...
                    minuteData = addDataFlow(minuteData, data, epCache.getTCache());
                    map.put(multikey, minuteData);
                    ...
                    hourData = addDataFlow(hourData, data, epCache.getTCache());
                    map.put(multikey, hourData);
                    ...
                    dayData = addDataFlow(dayData, data, epCache.getTCache());
                    map.put(multikey, dayData);
                    ...
                    monthData = addDataFlow(monthData, data, epCache.getTCache());
                    map.put(multikey, monthData);
                    ...
                    yearData = addDataFlow(yearData, data, epCache.getTCache());
                    map.put(multikey, yearData);

                    epCache.setT(Double.parseDouble(data.getT() != null ? (data.getT() + "") : "0.0"));
                    epCache.setTCache(Double.parseDouble(data.getT() + ""));
                    mapCache.put(data.getDeviceId(), epCache);
                }
                ...
                log.info("[SAVE] UPDATE LASTTIME");
                dataPqsMapper.updateLastTime(schema, lastTime, tableYear.toString(),  Constants.Device_type.CAM_BIEN_LUU_LUONG);
            }
            log.info("!!![DONE]");
        }
    }
...
//                    log.error(
//                        "!!!Please insert the first record to [s3m_data_pqs] with [view_type] = 0  [view_time] = 0 and [sent_date] = 2023");
//                    continue;
//                }
//            } catch (Exception e) {
//                log.error("!!!SCHEMA [" + schema + "] IS NOT EXIST");
...
//    }
    ...
    public DataPqs addDataFlow(DataPqs dataPqs, DataFlow data, Double tCache) {
    	...
    	Double tDiff = 0.0d;
        if (data != null) {
            t = data.getT() != null ? data.getT() : 0.0d;
        }

        Double tPds = dataPqs.getT() != null ? dataPqs.getT() : (double) 0;
        Double tCa = tCache != null ? tCache : (double) 0;

        if (t > 0 && tCa != 0) {
            dataPqs.setT(tPds + (data.getT() - tCa));
            tDiff = (double) (data.getT() - tCa);
        } else {
            dataPqs.setT(tPds);
        }
        return dataPqs;
